...
@section('content')
    <div class="page-heading">
        <div><p class="eyebrow">Assessor Module</p><h1>Assessments</h1><p>{{ auth()->user()->role === 'reviewer' ? 'Assessments assigned to you for review.' : 'Manage your drafts, follow review progress, and access your results.' }}</p></div>
    ...
    <div class="card">
        ...
        @if($assessments->isEmpty())
            <div class="empty-state"><x-icon name="document" size="36" /><h2>No assessments yet.</h2><p>{{ auth()->user()->role === 'reviewer' ? 'Your assigned assessments will appear here.' : 'Start a new assessment to work through the NCSB baseline.' }}</p></div>
        @else
            <div class="table-responsive" role="region" aria-label="Assessment register" tabindex="0">
                <table class="table align-middle"><thead><tr>
                    <th scope="col">Assessment</th>@if(auth()->user()->isAdmin() || auth()->user()->role === 'reviewer')<th scope="col">Assessor</th>@endif
                    ...
                    @foreach($assessments as $assessment)
                        <tr><td><a class="table-link" href="{{ route('assessments.show', $assessment) }}">#{{ $assessment->id }}</a></td>
                            @if(auth()->user()->isAdmin() || auth()->user()->role === 'reviewer')<td>{{ $assessment->user->name }}</td>@endif
